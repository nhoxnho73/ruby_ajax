class Appointment < ApplicationRecord
  ...
  class AppointmentTimeValidator
    ...
    def validate
      # select the user`s appointments from yesterday, today and tomorrow
      appointments = @user.appointments.select { |a| a.appointment_time.midnight == @appointment.appointment_time.midnight || a.appointment_time.midnight == @appointment.appointment_time - 1.day || a.appointment_time.midnight == @appointment.appointment_time - 1.day || a.appointment_time.midnight == @appointment.appointment_time  + 1.day }

      # makes sure that current appointment don`t overlap
